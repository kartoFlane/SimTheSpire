...
public abstract class TargetSingleCardTemplate<S extends CardState> extends CardTemplate<S> {
    ...
    public final CardPileType onPlay(GameController gameController, EntityController caster, List<EntityController> targets, S state) {
        return onPlay(gameController, caster, targets.get(0), state);
    }

    public abstract CardPileType onPlay(GameController gameController, EntityController caster, EntityController target, S state);
}
